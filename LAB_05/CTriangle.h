...
public:
	...
	friend std::istream& operator>>(std::istream& in, CTriangle& tri);
	friend std::ostream& operator<<(std::ostream& out, const CTriangle& tri);
public:
	static CTriangle* Parse(std::string line);
};
