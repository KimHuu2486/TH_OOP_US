...
void Farm::soundFarm() {
	for (int i = 0; i < cattle.size(); i++) {
		if (cattle[i]->getKind() == 0) {
			Cow* cow = static_cast<Cow*>(cattle[i]);
			cow->sound();
		}
		else if (cattle[i]->getKind() == 1) {
			Sheep* sheep = static_cast<Sheep*>(cattle[i]);
			sheep->sound();
		}
		else if (cattle[i]->getKind() == 2) {
			Goat* goat = static_cast<Goat*>(cattle[i]);
			goat->sound();
		}
	}
}
...
std::map<std::string, int> Farm::giveBirthFarm() {
	...
	std::vector<Cattle*> babies;
	for (int i = 0; i < cattle.size(); i++) {
		int children = cattle[i]->giveBirth();
		if (children > 0) {
			if (cattle[i]->getKind() == 0) {
				for (int j = 0; j < children; j++) {
					babies.push_back(new Cow()); 
				}
				birthCount["Cow"] += children;
			}
			else if (cattle[i]->getKind() == 1) {
				for (int j = 0; j < children; j++) {
					babies.push_back(new Sheep());
				}
				birthCount["Sheep"] += children;
			}
			else if (cattle[i]->getKind() == 2) {
				for (int j = 0; j < children; j++) {
					babies.push_back(new Goat());
				}
				birthCount["Goat"] += children;
			}
		}
	}

	cattle.insert(cattle.end(), babies.begin(), babies.end());
...
std::map<std::string, int> Farm::giveMilkFarm() {
	...
	for (int i = 0; i < cattle.size(); i++) {
		if (cattle[i]->getKind() == 0) {
			Cow* cow = static_cast<Cow*>(cattle[i]);
			milkCount["Cow"] += cow->giveMilk();
		}
		else if (cattle[i]->getKind() == 1) {
			Sheep* sheep = static_cast<Sheep*>(cattle[i]);
			milkCount["Sheep"] += sheep->giveMilk();
		}
		else if (cattle[i]->getKind() == 2) {
			Goat* goat = static_cast<Goat*>(cattle[i]);
			milkCount["Goat"] += goat->giveMilk();
		}
	}
	return milkCount;
}
...
Farm::~Farm() {
	for (int i = 0; i < cattle.size(); i++) {
		if (cattle[i]->getKind() == 0) {
			delete static_cast<Cow*>(cattle[i]);
		}
		else if (cattle[i]->getKind() == 1) {
			delete static_cast<Sheep*>(cattle[i]);
		}
		else if (cattle[i]->getKind() == 2) {
			delete static_cast<Goat*>(cattle[i]);
		}
		else {
			delete cattle[i];
		}
